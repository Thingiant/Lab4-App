...
export const PhotoCarousel = ({ photos, carouselHeight = 200, dotsStyle = {} }: Props): React.ReactElement => {
			...
			{carouselWidth > 0 && (
				<View style={[styles.carouselWrapper, { width: carouselWidth, height: carouselHeight }]}>
					...
					<View style={[styles.dotsContainer, RNStyleSheet.flatten(dotsStyle)]} accessible={false}>
						{photos.map((_, index) => (
							<AnimatedDot key={index} index={index} scrollX={scrollX} carouselWidth={carouselWidth} />
						))}
					</View>
				</View>
			)}
