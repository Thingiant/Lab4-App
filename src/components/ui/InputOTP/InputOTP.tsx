...
export const InputOTP = ({ onChange, error = false }: Props) => {
	...
	const inputs = useMemo(
		() =>
			Array(config.length)
				...
				.map((_, index) => {
					...
					return (
						<Pressable key={index} onPress={handlePress} style={[styles.codeInputItem, isFocused && styles.focusedInput, error && styles.errorInput]}>
							{char && shouldAnimate ? (
								<Animated.View entering={FadeInDown.duration(animationConfig.fadeIn.duration).easing(animationConfig.fadeIn.easing)}>
		...
		[code, focusedIndex, handlePress, shouldAnimate, error, styles.codeInputItem, styles.focusedInput, styles.errorInput]
	);
