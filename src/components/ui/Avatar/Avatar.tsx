...
export const Avatar = ({
	...
	const hasValidSource = source && (typeof source === 'number' || !('uri' in source) || (source.uri && source.uri.trim() !== ''));

	return (
		<View
			style={[styles.container({ size }), (!hasValidSource || !imageLoaded) && styles.containerColor({ color: fallbackColor })]}
			accessible
			...
			<View style={styles.fallbackContainer}>
				...
				{hasValidSource && (
					<Image source={source} style={[styles.image, { opacity: imageLoaded ? 1 : 0 }]} onLoad={() => setImageLoaded(true)} alt={alt} />
				)}
			</View>
