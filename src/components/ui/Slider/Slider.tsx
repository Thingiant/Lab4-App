...
export const Slider = ({
	...
	const adjustValue = useCallback(
		(action: 'increment' | 'decrement') => {
			...
			const newValue = action === 'increment' ? Math.min(max, currentValue + accessibilityStep) : Math.max(min, currentValue - accessibilityStep);
		...
		[getSliderValue, position, max, accessibilityStep, min, getPositionFromValue, onValueChange, animationConfig.position.spring]
	);
	...
	const panGesture = Gesture.Pan()
		...
		.onTouchesDown(() => {
			'worklet';
			knobScale.value = withTiming(animationConfig.scale.activeKnobScale, animationConfig.scale.timing);
		})
		.onTouchesUp(() => {
			'worklet';
	...
	return (
		<View style={styles.container}>
			<View style={styles.slider(sliderWidth)}>
				<Animated.View key={`slider-fill-${UnistylesRuntime.themeName}`} style={[styles.fill, fillStyle]} />
