...
export const SliderDual = ({
	...
	const leftPosition = useSharedValue(minInitialValue !== undefined ? getPositionFromValue(minInitialValue) : 0);
	const rightPosition = useSharedValue(maxInitialValue !== undefined ? getPositionFromValue(maxInitialValue) : sliderWidth);
	...
	useAnimatedReaction(
		...
		(current, previous) => {
			if (current.dragging && (current.leftPos !== previous?.leftPos || current.rightPos !== previous?.rightPos)) {
	...
	const createKnobGesture = useCallback(
		({
			...
			Gesture.Pan()
				...
				.onTouchesDown(() => {
					'worklet';
					scale.value = withTiming(animationConfig.scale.activeKnobScale, animationConfig.scale.timing);
				})
				.onTouchesUp(() => {
					'worklet';
	...
	return (
		<View style={styles.container}>
			<View style={styles.slider(sliderWidth)}>
				<Animated.View key={`slider-dual-fill-${id}-${UnistylesRuntime.themeName}`} style={[styles.fill, fillStyle]} />
