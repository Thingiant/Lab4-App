...
import { AccessibilityInfo, AccessibilityProps, LayoutChangeEvent, Modal, TouchableWithoutFeedback, useWindowDimensions, View } from 'react-native';
...
export const BottomSheet = ({
	...
	const bottomSheetMaxHeight = useMemo(() => Math.max(maxHeight ?? 0, windowHeight), [maxHeight, windowHeight]);
	...
	useEffect(() => {
		if (contentHeight) {
			translateY.value = visible ? withSpring(0, animationConfig.sheetOpen) : withTiming(contentHeight, animationConfig.sheetClose);
			overlayOpacity.value = withTiming(visible ? 0.5 : 0, visible ? animationConfig.sheetClose : animationConfig.sheetClose, () => {
				if (!visible) {
	...
	const gesture = Gesture.Pan()
		...
		.onUpdate((event) => {
			...
			if (newTranslateY >= 0) {
				...
				overlayOpacity.value = withTiming(0.5 * (1 - Math.min(newTranslateY / (contentHeight ?? 200), 1)), animationConfig.overlayDrag);
			}
		})
		.onEnd((event) => {
	...
	return (
		<Modal accessible={false} transparent={true} visible={showModal} onRequestClose={onRequestClose} animationType="none">
			<GestureHandlerRootView>
				<GestureDetector gesture={gesture}>
					<View style={styles.container}>
						<Animated.View style={[styles.overlay, StyleSheet.absoluteFillObject, overlayAnimatedStyle]}>
